...
**5.1.2.3** To design a WebForm, you need to drag and drop the activities from Right pallet. 

Drop **Text Area**  activity to design fields for orderID,shippingState,model,returnReason,status.

Drop **Number** Activity For unitPrice,noOfitems

Update the label as per data items.
...
It is three step process.Perform steps as shown
...
**5.1.3.6** Next from same page here navigate to the **Forms** section,click on **Search** icon  and select previously created  Webform  as shown.Click **OK** to close the tab.
